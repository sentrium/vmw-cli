...
// get client - replace request module with getFile
...
module.exports.getProduct = async function getProduct(solution) {
		...
		final = await loadMain(); // restructure promise architecture
...
function startProduct(name) {
	...
	if(fs.existsSync(indexDir + '/fileIndex.json')) {
		fileData = require(indexDir + '/fileIndex.json');
	}
...
function queueIdle(data, name) {
	...
	fs.writeFileSync(indexDir + '/fileIndex.json', JSON.stringify(mainIndex, null, "\t"));
